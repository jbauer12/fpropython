...
def terminal(game_board: GameBoard) -> bool:
    ...
    number_red_pieces = all(piece.team =="R" or piece.team ==" " for row in game_board.game_board for piece in row) 
    number_green_pieces = all(piece.team =="G" or piece.team ==" " for row in game_board.game_board for piece in row) 
...
def result(game_board: GameBoard, action: Tuple[Tuple[int, int], Tuple[int, int]]):
    if not terminal(game_board=game_board) and action:
